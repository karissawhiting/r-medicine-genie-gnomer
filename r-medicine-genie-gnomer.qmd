...
1)  [Process data]{.emphasized} into a analysis-ready matrix of events.
...
::: {.callout-note appearance="simple"}
2)  [Cohort Inclusion]{.emphasized}
    -   Samples with no alterations may be dropped when pulling data
:::

::: {.callout-note appearance="simple"}
3)  [Data Formats & Gene Standards Often Inconsistent]{.emphasized}
...
-   If patients only have one report, it will be returned regardless of criteria
:::
...
GENIE CNA and fusion data is slightly different than this standard, therefore we need to reformat it using:
...
- Additional [visualizations]{.emphasis} and [color palettes]{.emphasis} useful for genomic data are available in {gnomeR} package
...
-   It may be appropriate to [oncoKB annotate]{.emphasized} your data and only analyze oncogenic mutations (see oncoKB.org for more information)
