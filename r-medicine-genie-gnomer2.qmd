...
nsclc_2$NSCLC_v2.0$ca_dx_index %>% select(institution, stage_dx) %>%
  tbl_cross()
```
:::
...
- if there isn't there and selection of anything they have
...
bin_mat <- gnomeR::create_gene_binary(
    ...
    fusion = gnomeR::reformat_fusion(nsclc_2$NSCLC_v2.0$fusions),
...
## Process Data: Sample/Panel pitfalls
...
positions <- c(1:3)
nsclc_2$NSCLC_v2.0$cna %>% select(positions) %>% head() %>% gt::gt()
...
reform_fus <- gnomeR::reformat_fusion(nsclc_2$NSCLC_v2.0$fusions)
...
reform_fus %>% head() %>% select(all_of(c(1, 2, 3, 6))) %>% gt::gt()

```

## Process Data: Gene Aliases {.scrollable}
